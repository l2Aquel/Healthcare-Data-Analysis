--EASY LEVEL 
--Focus: SELECT, WHERE, GROUP BY, ORDER BY, LIMIT/TOP AND Basic Joins.

-- Q1 Patient Demographics: Provide a count of patients grouped by their RACE,GENDER,MARITAL STATUS AND ETHINICITY.
...
--Q2 Patients Check: List the cities and no of patients from each city from high to low
SELECT city,COUNT(*) AS total_patients
FROM [hospital_db].[dbo].[patients]
GROUP BY city
ORDER BY total_patients DESC;

--Q3 Encounters over the years: Find the no of encounters over the years and order them according to years older to recent
SELECT YEAR(start) as year,COUNT(*) AS total_encounters 
FROM [hospital_db].[dbo].[encounters]
GROUP BY  YEAR(start)
ORDER BY YEAR(start);

--Q4 Top Healthcare Payers: List the Payers that have covered the highest number of encounters AND also the highest amount of encounters
...
--Q5 Specific Condition Search: Find all patients (First and Last name) who have a REASONDESCRIPTION of 'Hyperlipidemia'.
...
--MEDIUM LEVEL
--Focus: Joins, Coalesce, CASE statements, Date functions, and Aggregations.

--Q6 Patient Lifecycle Segmentation: Categorize the entire patient population into three age-based segments: 
...
--Q7 Cost Discrepancy: Calculate the difference between TOTAL_CLAIM_COST and PAYER_COVERAGE. List the Patient where the "Out of Pocket" amount is more than 50000.
...
--Q8 Stay Duration: Calculate the average length of stay (in minutes) for each ENCOUNTERCLASS.
SELECT encounter_class,AVG(DATEDIFF(MINUTE,start,stop)) AS total_stay
FROM [hospital_db].[dbo].[encounters]
GROUP BY encounter_class
ORDER BY total_stay DESC;

--Q9 High-Volume Regional Hub Analysis: Identify the major geographic hubs by listing all cities that have surpassed a threshold of 500 total healthcare encounters.
...
--Q10 Payer Revenue Attribution Analysis: Calculate the total revenue generated from each healthcare insurance provider to identify the hospital's primary financial contributors.
...
--HARD LEVEL
--Focus: Window Functions(RANKING,AVG,LAG/LEAD), CTEs and Complex Filters.

--Q11 Financial Trend Analysis: Calculate a 7-Day Moving Average of the total daily healthcare revenue for the first quarter of 2021.
...
--Q12 Ranked Expenses: Within each CITY, rank patients by their total healthcare spend from highest to lowest. Show only the top 3 patients per city.
...
--Q13 Month-over-Month Revenue: Using a CTE, calculate the total TOTAL_CLAIM_COST per month for the year 2021, and show the percentage growth compared to the previous month.
...
--Q14 Retention & Gap Analysis: Identify "Lapsed Patients" who had a gap of more than 2 years (730 days) between consecutive healthcare visits.
...
--Q15 Patient Population Stratification & Revenue Analysis: Segment the patient database into three financial tiers 
